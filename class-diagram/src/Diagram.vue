    ...
    methods: {
        ...
        importDiagram(event) {
            ...
            reader.onload = () => {
                ...
                root.updateMnmxXy()          // import
                root.$nextTick(this.drawInCnvs) // import
                event.target.value = ''
            };
            reader.onerror = () => {
        ...
        changeLanguage(val) {
            ...
            for (let ii = 0; ii < langs.length; ii++) {
                if (langs[ii] == val) {
                    root.PLang = ii
                    root.updateMnmxXy()         // change lang
                    root.$nextTick(this.drawInCnvs) // change lang
                    break;
                }
            }
        },
    ...
    <div style="display: flex;" :minx="$root.MinX" :maxx="$root.MaxX" :miny="$root.MinY" :maxy="$root.MaxY">
        <aside id="dnb-vwtab-left" :style="[{ width: $root.MinX + 'px' }]">
            ...
            <div style="display: inline-block;border: 1px solid rgb(0, 0, 0);
                width: calc(100% - 10px);cursor: default;border-radius: 4px;" @mousedown="msDwnNewCls" id="cls-sample">
        ...
        <article id="dnb-vw-main" :style="{ height: VwHeight }">
