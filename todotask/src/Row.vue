...
const availRegions = computed(() => { return store.filter((r) => props.item.RegionIds.has(r.Id)) });
...
<template>
    <div class="nodewrap rounded-md px-1 shadow bg-white min-w-[120px] min-h-[46px] w-full" :level="Level">
        <div class="grid grid-cols-4 gap-0 items-center h-[46px]">
            <div class="inline-flex gap-[6px] items-center" :style="{ paddingLeft: Level * 6 + 'px' }">
                ...
                <div class="cursor-default min-w-12 text-lg truncate" @click.stop="clickName">{{ item.Name }}</div>
            </div>
            <div class="inline-flex gap-1 cursor-default text-sm">
                <div v-for="r in availRegions" class="rounded-md bg-neutral-200 px-1.5 py-0.5">{{ r.Name }}</div>
            </div>
            <div class="inline-flex gap-1 cursor-default text-sm">
                <div v-for="r in availUsers" class="rounded-md bg-neutral-200 px-1.5 py-0.5">{{ r.Name }}</div>
            </div>
        </div>
    </div>
</template>